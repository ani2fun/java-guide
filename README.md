...
The **Java** book for [Synapse](https://synapse.kakde.eu). This repository *is* the book: the
chapters sit at the root, and Synapse mounts it at `/synapse/programming-languages/java`.
...
- **Chapters are directories, lessons are `.md` files.** Ordering comes from the `NN-` prefix
  ...
  becomes `/synapse/programming-languages/java/first-steps/variables`. An `index.md` sorts first; anything unnumbered sorts last,
  alphabetically.
- **Where the book sits in the library is Synapse's business, not this repo's.** It is registered
  from the admin panel, which is what places it under the **programming-languages** group.
...
Assets live at `_media/java/<lesson-slug>/…` and are referenced app-absolutely:
`![alt](/media/java/<lesson-slug>/diagram.svg)`. Supported: svg, png, webp, jpg, gif, mp4, webm.

## Links

Intra-book links are app-absolute and are **not** rewritten: `/synapse/programming-languages/java/<chapter>/<lesson>`. Relative
